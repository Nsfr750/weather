...
async def test_plugin_system():
    ...
    test_plugins_dir = script_dir.parent / "plugins" / "weather_providers"
    plugin_manager = PluginManager([test_plugins_dir])
    
    # Verify the plugin directory exists
    assert test_plugins_dir.exists(), f"Plugin directory not found: {test_plugins_dir}"
    ...
    # Verify at least one plugin was loaded
    assert len(plugin_manager.plugins) > 0, "No plugins were loaded"
    
    # List discovered plugins
    logger.info("Discovered plugins:")
    for plugin_name, plugin_class in plugin_manager.plugins.items():
        logger.info(f"- {plugin_name} (version: {getattr(plugin_class, 'version', 'unknown')})")
    
    # Test OpenMeteo plugin (should be available without API key)
    test_plugin_name = 'openmeteo'
    if test_plugin_name in plugin_manager.plugins:
        logger.info(f"\nTesting {test_plugin_name} plugin...")
        plugin_class = plugin_manager.plugins[test_plugin_name]
        
        # Get weather provider instance
        try:
            provider = plugin_class.get_weather_provider()
            ...
            logger.error(f"Failed to initialize {test_plugin_name} provider: {e}")
    else:
        logger.warning(f"{test_plugin_name} plugin not found in discovered plugins")
